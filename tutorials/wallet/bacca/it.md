---
nome: Bacca
descrizione: Configurazione di un Ledger senza il software Ledger Live
---
...
In questo tutorial ti mostrerò come utilizzare questo strumento, in modo da poter fare definitivamente a meno del software Ledger Live e continuare a utilizzare i dispositivi Ledger. Funziona su tutti i dispositivi: Nano S Classic, Nano S Plus, Nano X, Flex e Stax.
...
Se sono già installati, puoi saltare questo passaggio.

**Linux:**

Su una distribuzione Linux, Git è generalmente preinstallato. Per verificare se Git è installato sul tuo sistema, digita il seguente comando nel terminale:

```bash
git --version
```

Se non hai Git installato sul tuo sistema, ecco il comando per installarlo su Debian:

```bash
sudo apt install git
```

Infine, per installare l'ambiente di sviluppo Rust, utilizza il comando :
...
Per installare Git, visita [il sito web ufficiale del progetto](https://git-scm.com/). Scaricare il software e seguire le istruzioni di installazione.

![BACCA](assets/fr/01.webp)

Procedere allo stesso modo per installare Rust da [il sito ufficiale](https://www.rust-lang.org/tools/install).

![BACCA](assets/fr/02.webp)

**MacOS:**

Se Git non è ancora installato sul tuo sistema, apri un terminale ed esegui il seguente comando per installarlo:

```bash
git --version
```

Se Git non è installato sul tuo sistema, si aprirà una finestra che ti proporrà di installare Xcode, che include Git. È sufficiente seguire le istruzioni sullo schermo per procedere all'installazione.
...
Apri un terminale e vai nella cartella in cui vuoi salvare il software ed esegui il seguente comando:
...
Utilizzare Cargo per compilare il progetto ed eseguire la GUI di Bacca:
...
Bacca ti informerà se il tuo Ledger è autentico. Se non lo è, significa che il dispositivo è stato compromesso o che è contraffatto. In questo caso, smettete immediatamente di usarlo.
...
Per installare l'applicazione Bitcoin, fare clic su "*Install*" ed autorizza l'installazione sul tuo Ledger.
...
Se non è installata l'ultima versione dell'applicazione Bitcoin, Bacca visualizzerà un pulsante "*Update*" invece dell'indicazione "*Latest*". È sufficiente fare clic sul pulsante per aggiornare l'applicazione.

![BACCA](assets/fr/10.webp)

Ora che il tuo Ledger è configurato correttamente con l'ultima versione dell'applicazione Bitcoin, sei pront ad importare e utilizzare il tuo wallet su software di gestione come Sparrow o Liana, senza dover passare per Ledger Live!

Se hai trovato utile questa guida, ti sarei grato se lasciassi un pollice verde qui sotto. Sentiti libero di condividere questo articolo sui tuoi social network. Grazie mille!

Ti consiglio anche di dare un'occhiata a questo tutorial su GnuPG, che spiega come verificare l'integrità e l'autenticità del software prima di installarlo. Si tratta di una pratica importante, soprattutto quando si installa un software wallet come Liana o Sparrow:
